...
public class ui_respawn_button : MonoBehaviour
{
	...
	{
		//누르면 목숨 달고 플레이어 살아나고 생명 스프라이트 끔 다시살아나면 원위치에서
		if (player.GetComponent<player_move> ().stat == false) {
			life_point--;
			if (life_point >= 0) {
				life [life_point].SetActive (false);
